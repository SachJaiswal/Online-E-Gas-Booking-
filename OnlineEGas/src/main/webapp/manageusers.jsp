...
<%
    ...
    int counter = (int) currentSession.getAttribute("counter");
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
	...
	<div class="container" style="padding: 20px;">
		...
		<div id="manageusers" class="form-container ">
			<h2>
				<i class="fas fa-info-circle"></i> User List
			</h2>
			...
			<table class="table">
                ...
                <tbody>
                    <% for (int i = 1; i < counter; i++) { %>
                        <tr>
                            <td><%= i %></td>
                            <td><%= currentSession.getAttribute("user" + i + "Username") %></td>
                           
                            <td><%= currentSession.getAttribute("user" + i + "Mobile") %></td>
                            <td><%= currentSession.getAttribute("user" + i + "Email") %></td>
                            <td><%= currentSession.getAttribute("user" + i + "Address") %></td>
                            <td><%= currentSession.getAttribute("user" + i + "Aadhaar") %></td>
                            <td>
                                <form action="deleteUser" method="post">
                                    <input type="hidden" name="userId" value="<%= currentSession.getAttribute("user" + i + "Id") %>">
